...
boolean timedOut2 = true;
boolean special = false;// set to true when timer fired
...
void loop() {
  ...
  if ((userAnswer == false) && (timedOut == false) && (timedOut2 == false) && ((millis() - timer) > INTERVAL2)) {
    switch (answer.toInt()) {
      case 31:
  ...
  if ((userAnswer == false) && (timedOut == false) && ((millis() - timer) > INTERVAL)) {
    switch (answer.toInt()) {
      case 21:
        servo3.write(turnAngle);
        delay(800);
